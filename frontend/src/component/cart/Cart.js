...
const Cart = () => {
  ...
  return (
    <Fragment>
      ...
      {
        cartItems && cartItems.length === 0  || cartItems === 0 ? <div className="flex flex-col justify-center items-center w-[100vw] h-[100vh] max-w-full "><RemoveShoppingCartIcon className="text-[tomato] text-[10vmax]"/><p className="my-[2vmax]">No Product in Your Cart</p> <Link to='/products' className="bg-[#333333] text-white p-[12px_34px]">View Products</Link> </div> : (<Fragment>
          ...
          <div className="cartPage">
            ...
            <div className="cartGrosseProfit grid grid-cols-[0_2fr] md:grid-cols-[2fr_1.2fr]  w-[90%] m-auto">
              ...
              <div className="cartGrosseProfitBox flex justify-between border-t-[3px] border-t-[tomato] mt-7 py-5">
                ...
                <p> ৳{cartItems && cartItems.reduce((acc,item)=> acc + item.price * item.quantity,0)}</p>
              </div>
              <div></div>
